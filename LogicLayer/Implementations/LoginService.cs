...
public class LoginService: ILoginService
{
    ...
    public List <IOrderStatus> FindOrders()
    {
        if (_supplierLogged)
        {
            var orders = _dataRepository.GetOrderStatuses().FindAll(p => p.Order.Product.Supplier == _supplier);
            return orders;
        } else if (_shopLogged)
        {
            var orders = _dataRepository.GetOrderStatuses().FindAll(p => p.Order.Shop == _shop);
            return orders;
        }
        else throw new Exception("Not logged to a supplier");
    }

    public IOrder FindOrderById(string id)
    {
        if (_supplierLogged)
        {
            var order = _dataRepository.GetOrdersStatuses().Find(p => p.OrderId == id
                                                                      && _supplier == p.Order.Product.Supplier);
            if (order == null) throw new NullReferenceException("Order not found.");            
            return order;
        } else if (_shopLogged)
        {
            var order = _dataRepository.GetOrdersStatuses().Find(p => p.OrderId == id && p.Order.Shop == _shop);
            if (order == null) throw new NullReferenceException("Order not found.");
            return order;
        }
    ...
    public List<IOrderStatus> FindOrdersByStatus(OrderStatusEnum status)
    {
        if (_supplierLogged)
        {
            var orders = _dataRepository.GetOrderStatuses().FindAll(p => p.Status == status);
    ...
    public void SetStatus(string orderStatusId, OrderStatusEnum status)
    {
        ...
        var newOrderStatus = _dataRepository.GetOrderStatuses().Find(p => p.OrderStatusId == orderStatusId)
    ...
    public void SetStatus(string orderStatusId, string status)
    {
        ...
        var order = _dataRepository.GetOrderStatuses().Find(p => p.OrderStatusId == orderStatusId);
        if (order != null)
        {
    ...
    public void SetPending(string orderStatusId)
    {
        ...
        var orderStatus = _dataRepository.GetOrderStatusById(orderStatusId);
        if (orderStatus != null)
        {
